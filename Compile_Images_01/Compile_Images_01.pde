...
//String path = "C:\\Users\\cportelli\\Pictures\\2015_08_02_Sunset_Time_Lapse";
String path = "C:\\Users\\cportelli\\Pictures\\2015_08_15_Venice_Beach_Sunset";
...
void GetPixelsVertically() {
  ...
  float step = w / files.size();                                 // step size
  ...
  for (int i = 0; i < files.size (); i = i + ImageStep)          // go through all the images
  {
    ...
    int start = (int)(i * step);                                 // set the start and stop values for pixesl to compile
    int stop = (int)((i * step) + (step * ImageStep));

    if (PrintStats) {
      println("Loaded Image: " + files.get(i).getName() + " | Image Number: " + i + " | Number of Pixels: " + img.pixels.length);
...
void GetPixelsHorizontally() {
  ...
  int step = newImg.pixels.length/ files.size() ;
  ...
  for (int i = 0; i < files.size (); i = i + ImageStep) {
    PImage img = loadImage(files.get(i).getPath());    // load the image
    int start = i * step;
    int stop = (i * step) + (step * ImageStep);

    if (PrintStats) {    
      println("Loaded Image: " + files.get(i).getName() + " | Image Number: " + i);
      println("Start: " + start + " | Stop: " + stop);
    }
